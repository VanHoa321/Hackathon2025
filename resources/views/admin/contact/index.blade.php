                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($items as $item)
                                            <tr id="contact-{{ $item->id }}">
                                                ...
                                                <td>
                                                    <span class="badge badge-{{ $item->is_read ? 'success' : 'warning' }} px-3 py-2 status-badge" data-id="{{ $item->id }}">
                                                        {{ $item->is_read ? 'Đã đọc' : 'Chưa đọc' }}
                                                    </span>
                                                </td>
                                                <td>
                                                    <a href="{{ route('admin.contact.show', $item->id) }}" class="btn btn-info btn-sm" title="Xem chi tiết">
                                                        <i class="fa-solid fa-eye"></i>
                                                    </a>
                                                    <a href="#" class="btn btn-{{ $item->is_read ? 'secondary' : 'primary' }} btn-sm btn-mark-read" data-id="{{ $item->id }}" title="{{ $item->is_read ? 'Đánh dấu chưa đọc' : 'Đánh dấu đã đọc' }}">
                                                        <i class="fa-solid {{ $item->is_read ? 'fa-envelope' : 'fa-envelope-open' }} mark-icon"></i>
                                                    </a>
