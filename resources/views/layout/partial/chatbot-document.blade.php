...
<button id="toggleModal" class="btn rounded-circle position-fixed bottom-0 end-0 m-4" style="width: 60px; height: 60px; z-index: 9999; background-color:#11B76B; color:#FFFFFF">
    <i class="fas fa-comments"></i>
    <i class="fas fa-times" style="display: none"></i>
</button>

<div id="modalContainer" class="position-fixed bg-white rounded-3 shadow d-flex flex-column overflow-hidden" style="visibility: hidden; opacity: 0; transform: scale(0.95); transition: all 0.4s ease;">
    <header class="d-flex align-items-center justify-content-between px-3 py-2 border-bottom">
        <button id="zoomModal" aria-label="Zoom" class="btn btn-link p-0 text-dark">
        ...
        <button id="minimizeModal" aria-label="Minimize" class="btn btn-link p-0 text-dark">
            <i class="fas fa-minus"></i>
        </button>
    </header>

    <main id="chatbotDocumentContent" class="flex-grow-1 overflow-auto chat-body px-3 py-4">
    ...
    <div class="d-flex align-items-center border border-primary rounded-pill mx-3 my-3 px-3 py-2">
        <input id="chatBotDocumentInput" type="text" class="form-control border-0 me-2 custom-no-focus" placeholder="Nhập câu hỏi với tài liệu..." aria-label="Write a message" />
